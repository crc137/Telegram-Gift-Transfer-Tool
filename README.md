...
This tool is designed for transferring gifts using the Telegram Bot API. The script allows transferring stars from a business account to a bot and then using these stars to transfer gifts to users.
...
```
python telegram_gift_transfer.py
```

You can also provide a configuration file:
```
python telegram_gift_transfer.py --config my_config.json
```

### Web Interface

This tool includes a web interface built with Flask and Tailwind CSS, which allows you to:
- Configure settings through a user-friendly form
- Run the script and view real-time console output
- Download logs from current or previous runs
- View a list of all previous logs
...
1. **Configuration Form**: Enter your Bot Token, Business Connection ID, Target Chat ID, and Star Count.
2. **Console Output**: Watch real-time output from the script execution.
3. **Log Management**: Download current or previous log files.
4. **Previous Logs**: View and download logs from previous runs.
